...
for file in r.sample(audio_files, len(audio_files)):
    for transcript_type in ["gold", "asr"]:
        ...
        speaker_transcript = []
        aligned_filename = f"{segment_id}_{transcript_type}.pickle"
        ...
        for (word, begin, end) in data:
            ...
            # play(audio_file)
        pause_file = os.path.join("data", "penn_files", "pause_features", f"{segment_id}_{transcript_type}.pickle")
        pickle.dump(speaker_pause, open(pause_file, 'wb'))
        transcript_file = os.path.join("data", "penn_files", "transcripts", f"{segment_id}_{transcript_type}.txt")
